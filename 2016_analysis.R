...
Silver, a statistician who had a track record of making correct predictions on presidential election outcomes.

We will use two datasets: aug_poll.Rdata and 2016_election_result.csv in this project.
The dataset aug_poll.Rdata is a subset of the dataset downloaded from this website (see reference here).
aug_poll.Rdata contains poll data collected from polls that closed between Aug. 1, 2016 and Oct. 27, 2016.
This dataset was originally posted on fivethirtyeight.com, a website created by Nate Silver. Note: Even
though Silver’s prediction missed the truth (everyone else did) Nate Silver’s model was the most robust one
among all the models published on New York Times for predicting the 2016 presidential election result.
```
# Why was the predicted probabilities of winning so off for the candidates?				     
## Section 1
### Part a
```
load("aug_poll.RData")
...
head(poll.oct2016)
```
# poll.oct2016 aggregates the data in oct_poll to the state level and expresses rawpoll_clinton and rawpoll_trump as proportions instead of percentages
...
## Section 3
### part a
# Created the following vectors and added them as the columns to election:
# std.unit: The number of SE units each element in poll.diff
# same.sign: A logical vector that shows TRUE if both poll.diff[i] and vote.diff[i] have the same sign
# significant: A logical vector that shows TRUE if the 95% CI for estimating (proportion of Trump supporters - proportion of Clinton supporters) does not cover 0
```
...
### part b
# How many of the 95% CIs for estimating (proportion of Trump supporters - proportion of Clinton supporters) mentioned in part a are
# away from 0? Among the CIs that are away from 0 how many of them predict the winning candidate for the
# state correctly? How many of the 95% CIs cover 0? Among the CIs that cover 0 how many of them predict
# the winning candidate for the state correctly?
```
...
### part c
# Among the states that you are confident about whom the state winning candidate would be, how many of these states are our predictions are wrong about
```
...
### part d
# Plotting a scatterplot and a histogram of std.unit
```
...
### part e
#  Testing the hypothesis that the errors for all 50 states plus D.C. are independent and that there is an equally likely chance for a combined poll difference to be greater or less than the actual election difference for the state
```
...
# Section 4
## part a
# The entries of diff.state.date are the aggregated data for (sample proportion of Trump supporters - sample proportion of Clinton supporters)
```
...
## part b
# Plotting flip states and the rest of the swing states
```
...
## part c
# Do the poll results look more or less correlated among the flip states?
